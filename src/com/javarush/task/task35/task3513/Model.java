...
public class Model {
    ...
    public void autoMove() {
        PriorityQueue<MoveEfficiency> moveQueue = new PriorityQueue<>(4, Collections.reverseOrder());

        addMoveEfficiencyToQueue(moveQueue, this::left);
        addMoveEfficiencyToQueue(moveQueue, this::right);
        addMoveEfficiencyToQueue(moveQueue, this::up);
        addMoveEfficiencyToQueue(moveQueue, this::down);

        MoveEfficiency bestMove = moveQueue.poll();
        if (bestMove != null) {
            bestMove.getMove().move();
        }
    }

    private void addMoveEfficiencyToQueue(PriorityQueue<MoveEfficiency> queue, Move move) {
        MoveEfficiency moveEfficiency = getMoveEfficiency(move);
        queue.add(moveEfficiency);
    }
}
