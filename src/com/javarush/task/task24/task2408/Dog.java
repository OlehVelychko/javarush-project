...
Класс DogPet использует 2 класса - SuperDog и Dog, разберись с getName1 в классе DogPet
...
public class Dog implements Pet {
     ...
     * создайте метод private String getName1(), который будет использовать логику двух классов - Dog и SuperDog.
     * Пусть этот метод возвращает имя собаки (класс Dog), которое с обеих сторон выделено getSuperQuotes (класс SuperDog)
     * Пример, " *** Барбос *** "
     * Логика метода say:
     * Если i < 1, то используя метод getName1 вывести на экран, что собака спит. Пример, " *** Шарик ***  спит."'
    ...
    public Sayable toSayable(final int i) {
        class DogPet extends SuperDog implements Sayable {
            ...
            public String say() {
                String result;
                if (i < 1) {
                    result = String.format("%s спит.", getName());
                } else {
                    char[] gav = new char[i];
                    Arrays.fill(gav, 'а');
                    result = String.format("%s лает г%sв! %s", getName(), String.valueOf(gav), formatter.format(new Date()));
                }
                return result;
            }
        }
